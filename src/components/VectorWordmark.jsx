...
const HANDLE_DEFAULTS = { size: 109, spread: 27, labels: false };

function BaseVectorWordmark(props) {
  const {
    text = "GOMAILER",
    font = {
      fontFamily: "Inter",
  ...
  const fontSpec = {
    family: font?.fontFamily || "Inter, system-ui, sans-serif",
    weight: String(font?.fontWeight ?? 800),
    style: font?.fontStyle === "italic" ? "italic" : "normal",
    size: Math.max(8, parseFloat(String(rawSize ?? 200)) || 200),
    letterSpacing: String(font?.letterSpacing ?? "-0.02em"),
  };

  const accentRGBA = parseColor(accent, [1, 1, 1, 0.35]);
  const labelColor = `rgb(${Math.round(accentRGBA[0] * 255)}, ${Math.round(accentRGBA[1] * 255)}, ${Math.round(accentRGBA[2] * 255)})`;

  const live = useRef({ text, fontSpec, textColor, shade, background, accentRGBA, reach, speed, damping, hg });
  ...
  useEffect(() => {
    ...
    const attrs = { alpha: true, antialias: false, depth: false, stencil: false, premultipliedAlpha: true, powerPreference: "high-performance" };
    ...
    let atlasRatioH = 1;
    let atlasKey = "";
    function drawFontPx() { return live.current.fontSpec.size * (boxW / REF_WIDTH); }
    function resize() {
    ...
    const cells = [];
    const verts = [];
    for (let i = 0; i < HANDLES; i += 1) { cells.push({ x: -0.5, y: 0.5 }); verts.push({ x: -0.5, y: 0.5 }); }
    ...
    function snap(x, y, cw, ch) {
      ...
      for (let i = 0; i < HANDLES; i += 1) { cells[i].x = found[i + 1].x; cells[i].y = found[i + 1].y; }
    }
    const onMove = (e) => {
      hasPointer = true;
    ...
    function sync() {
      ...
      const f = L.fontSpec;
      const key = [L.text, f.family, f.weight, f.style, f.letterSpacing, dpr, Math.ceil(Math.max(8, drawFontPx()) / 64)].join("|");
    ...
    function draw() {
      ...
      const sc = parseColor(L.shade, [0.4, 0.4, 0.45, 1]);
  ...
  return (
    <div ref={hostRef} style={{ position: "relative", overflow: "hidden", background, width: "100%", height: "100%", ...style }}>
      <canvas ref={canvasRef} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", display: "block" }} />
      {hg.labels ? [0, 1, 2].map((i) => (
        <div key={i} ref={(el) => { labelRefs.current[i] = el; }}
          style={{ position: "absolute", left: 0, top: 0, opacity: LABEL_MAX, pointerEvents: "none", whiteSpace: "nowrap", fontFamily: "ui-monospace, Menlo, monospace", fontSize: 11, letterSpacing: "0.08em", color: labelColor }} />
      )) : null}
    </div>
  );
}
...
export default function VectorWordmark(props) {
  return <BaseVectorWordmark handles={{ size: 109, labels: false, spread: 27 }} {...props} />;
}
